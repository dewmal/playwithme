...
export function ExportDialog({ close, videoPath, processingStatus }: { close: () => void; videoPath: string | null; processingStatus: string | null }) {
  ...
  const runVideo = async () => { if (!videoPath || processingStatus) return; setBusy("video"); setMessage(""); setError(""); try { if (await exportVideo(videoPath, presentationFile)) setMessage("Video exported successfully."); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(null); } };
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && close()}><section className="export-dialog">
    ...
    <div className="export-grid">
      ...
      <button className={`export-card${videoPath && !processingStatus ? "" : " disabled"}`} onClick={runVideo} disabled={!!busy || !!processingStatus || !videoPath}><span className="export-icon dark"><Film /></span><span><b>Session video</b><small>{processingStatus ?? (videoPath ? "Choose an MP4 name and destination" : "Saved as MP4 when a recorded session ends")}</small></span>{processingStatus || busy === "video" ? <LoaderCircle className="spin" /> : videoPath ? <span className="arrow">→</span> : <em>Record first</em>}</button>
    </div>
