...
export function ExportDialog({ close, videoPath, processingStatus }: { close: () => void; videoPath: string | null; processingStatus: string | null }) {
  ...
  const runVideo = async () => { if (!videoPath) return; setBusy("video"); setMessage(""); setError(""); try { if (await exportVideo(videoPath)) setMessage("Video exported successfully."); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(null); } };
