...
export function ExportDialog({ close, sectionCount, processingStatus, exportRecording }: { close: () => void; sectionCount: number; processingStatus: string | null; exportRecording: () => Promise<boolean> }) {
  ...
  const runVideo = async () => { if (!sectionCount || processingStatus) return; setBusy("video"); setMessage(""); setError(""); try { if (await exportRecording()) setMessage("Video exported successfully."); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(null); } };
